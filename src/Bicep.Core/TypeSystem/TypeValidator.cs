...
namespace Bicep.Core.TypeSystem
{
    public class TypeValidator
    {
        ...
        {
            static TypeSymbol GetPropertyAssignmentType(TypeSymbol propertyType, TypePropertyFlags propertyFlags) =>
                // TODO: Do we need a separate flag to prevent the appending of "| null"
                propertyFlags.HasFlag(TypePropertyFlags.Required) || propertyFlags.HasFlag(TypePropertyFlags.DisallowAny)
                    ? propertyType
                    : UnionType.Create(propertyType, LanguageConstants.Null);
            ...
            foreach (var declaredProperty in targetType.Properties.Values)
            {
                if (namedPropertyMap.TryGetValue(declaredProperty.Name, out var declaredPropertySyntax))
                {
                    ...
                    TypeSymbol propertyAssignmentType = GetPropertyAssignmentType(declaredProperty.TypeReference.Type, declaredProperty.Flags);
            ...
            else
            {
                ...
                foreach (var extraProperty in extraProperties)
                {
                    ...
                    var additionalPropertiesAssignmentType = GetPropertyAssignmentType(targetType.AdditionalPropertiesType.Type, targetType.AdditionalPropertiesFlags);
